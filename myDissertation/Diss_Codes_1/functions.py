...
def Extract_subject_folds(subject, script_dir, DS):
    ...
    rel_path = 'Datasets/Online_Dataset/DS' + str(DS) + '/S' + str(subject + 1) + '.txt'
...
def Evaluate_trial(Trial, Epochs, trial, scaler, Idx, FeatureSet, Clf, pca):
    ...
    TrialAccuracy = metrics.accuracy_score(Trial_corrected, ActualTrial)
    ...
    return TrialF1score, TrialAccuracy, TrialPrecision, TrialRecall
...
def Evaluate_fold(SubjectScores, TestTrials, TestEpochs, scaler, Idx, FeatureSet, Clf, fold, pca):
    ...
    for trial in range(len(TestTrials)): # Run trough each trial
        ...
        TrialF1score, TrialAccuracy, TrialPrecision, TrialRecall = Evaluate_trial(Trial, Epochs, trial, scaler, Idx, FeatureSet, Clf, pca)   
        
        # Writes the obtained metrics of the trial
        SubjectScores['TrialF1scores'][trial,fold] = TrialF1score
        SubjectScores['TrialAccuracy'][trial,fold] = TrialAccuracy
...
def Evaluate_subject(Results, subject, DS, FeatureSet, clf, Idx, script_dir):
    ...
    SubjectScores['TrialAccuracy'] = np.zeros((Size_train_trials,4))
    ...
    Results['Accuracy'].append(SubjectScores['TrialAccuracy'])
